...
function initMap() {
  var map = L.map('map').setView([48.9213845, 24.7205442], 14);
...
Array.from(document.getElementsByClassName('video_wrap')).forEach(function (el) {
  // const code= el.getAttribute('data-code');
  // console.log(el.dataset.code);
  el.innerHTML = "\n    <a href=\"#\" class=\"load_video\">\n    <img src=\"https://img.youtube.com/vi/".concat(el.dataset.code, "/maxresdefault.jpg\">\n    </a>\n    ");
});
document.body.addEventListener('click', function (e) {
  ...
  if (e.target.parentElement.classList.contains('load_video')) {
    e.preventDefault();
    e.stopPropagation();
    e.target.parentElement.parentElement.innerHTML = "\n        <iframe src=\"https://www.youtube.com/embed/".concat(e.target.parentElement.parentElement.dataset.code, "\" title=\"YouTube video player\" frameborder=\"0\"\n        allow=\"accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture\"\n        allowfullscreen></iframe>"); //* для video треба зберегти відео у себе і зробити декілька форматів(для кожного браузера свій формат). 
    // e.target.parentElement.parentElement.innerHTML =
    // `
