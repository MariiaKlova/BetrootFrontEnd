...
var liElements = document.querySelectorAll('.list li'); //! getElementsByTagName -Повертає живу колекцію і все спрацює
...
newLi.innerText = 'New Lorem ipsum dolor sit amet.'; //ToDo  newLi.onclick = liColorChangeHandler;  // При неживій колекції без цього не зпрацює
...
_toConsumableArray(liElements).forEach(function (li) {
...
function btnLiHendler() {
  var newLi = document.createElement('li'); //ToDo  newLi.onclick = liColorChangeHandler; // При неживій колекції без цього не спрацює
...
document.body.addEventListener('click', function (event) {
  ...
  if (event.target.nodeName === 'LI') {
    ...
    */
    liColor(event.target); // btnLiHendler(); // Можна додавати багато функцій
  }
...
function blockCopy(e) {
  if (e.ctrlKey && e.code === 'KeyC' || e.code === 'KeyU'
  /* || e.code === 'F12' */
  ) {
      e.preventDefault();
