...
const block_resize = document.createElement('div');
...
function text_editor(e) {
    if (e.ctrlKey && e.code === 'KeyE') {
        ...
        textarea.value = text_block.innerText; // «содержимое»
...
const mouseDownHandler = function (e) {
    ...
    // розмір єлемента
    const styles = window.getComputedStyle(text_block);
...
const mouseMoveHandler = function (e) {
    ...
    // Зміна розміру єлемента
    text_block.style.width = `${w + dx}px`;
    text_block.style.height = `${h + dy}px`;

    // запис їх у обєкт стилів
    style_shit.text_block.width = `${w + dx}px`;
    style_shit.text_block.height = `${h + dy}px`;
...
const resizers = text_block.querySelectorAll('.resizer');

// Застосування для кожного протягування мишею
