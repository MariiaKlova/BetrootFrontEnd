...
function sortTable(n) {
  ...
  switching = true;
  dir = "asc"; //Цикл, який триватиме до тих пір, поки не буде зроблено перемикання: 

  while (switching) {
    switching = false;
    rows = table.rows; // Перебрати всі рядки таблиці (крім першого, який містить заголовки таблиці): 

    for (i = 1; i < rows.length - 1; i++) {
      shouldSwitch = false; // Два елементи, які потрібно порівняти, один із поточного рядка, а інший із наступного: 
...
var arr = document.querySelectorAll('th');
